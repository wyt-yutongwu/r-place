...
def generate_user_signed_net(usr_rel_same_color, usr_rel_diff_color, user_index, output_file_name):
    ...
    with open(output_file_name+".txt", 'w') as file_out:
        ...
        for u, v in zip(*usr_rel_same_color.nonzero()):
            if u < v:
                value = int(usr_rel_same_color[u,v])
                value = value - usr_rel_diff_color[u,v]
                writer.writerow([u+1, v+1, value])
...
def sign_embedding(usr_rel_same_color, usr_rel_diff_color, user_index, output_file_name, 
    ...
    print("python2.7 "+signet_path+" -l signet_id.txt -i signet.txt -o "+output_file_name+ " -d "+str(ndim)+" -t "+str(threshold)+" -s "+str(total_samples))

    os.system("python2.7 "+signet_path+" -l signet_id.txt -i signet.txt -o "+output_file_name+ " -d "+str(ndim)+" -t "+str(threshold)+" -s "+str(total_samples))
...
def act_embedding(G, user_index, output_file_name, 
    ...
    n_iterations=10):
    '''
        Computes standard embedding based on user relations using the algorithm from: 
        SIGNet: Scalable Embeddings for Signed Networks, code from: 
        https://github.com/raihan2108/signet

	#python signet.py -l ids.txt -i input.txt -o output.txt -d 40 -t 5 -s 100
	#"python "+signet_path+" -l actnet_id.txt -i actnet.txt -o "+output_file_name+ " -d "+str(ndim)+" -t "+str(threshold)+" -s "+str(total_samples))
	3ndim=40, threshold=5, total_samples=100, n_negatives=5, n_iterations=10
    '''
    ...
    os.system("python "+signet_path+" -l actnet_id.txt -i actnet.txt -o "+output_file_name+ " -d "+str(ndim)+" -t "+str(threshold)+" -s "+str(total_samples))
