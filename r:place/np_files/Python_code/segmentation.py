...
def compute_user_activity_graph(G, ups, gcc=True, final=False, color=False): 
    user_index = {}
    user_list = []
    user_relations = {}
    ...
    with open(G.unique_edges_file_name, 'r') as file_in:
        ...
        for r in reader:
            ...
            if ((ups[u][6] == 1 and ups[v][6] == 1) or not final) and (color_u == color_v or not color) and type_edge > 0 and np.abs(x_u-x_v) <= 1 and np.abs(y_u-y_v) <= 1:
                if user_u not in user_index:
                    user_index[user_u] = len(user_index)
                    user_list.append(user_u)
                ...
                if idx_u < idx_v:
                    if (idx_u,idx_v) not in user_relations:
                        user_relations[(idx_u,idx_v)] = 0

                    user_relations[(idx_u,idx_v)] = user_relations[(idx_u,idx_v)] + 1
                elif idx_v < idx_u:
                    if (idx_v,idx_u) not in user_relations:
                        user_relations[(idx_v,idx_u)] = 0

                    user_relations[(idx_v,idx_u)] = user_relations[(idx_v,idx_u)] + 1

    G_act = nx.Graph()
    for e in user_relations:
        G_act.add_edge(e[0], e[1], weight=user_relations[e])

    if gcc is True:
        largest_cc = max(nx.connected_components(G_act), key=len)
        G_act = G_act.subgraph(largest_cc)
        ...
        for v in G_act.nodes():
            new_user_index[user_list[v]] = i
            index_map[user_index[user_list[v]]] = i
            i = i + 1

        for e in G_act.edges():
            new_G_act.add_edge(index_map[e[0]], index_map[e[1]], weight=G_act[e[0]][e[1]]['weight'])  

        return new_G_act, new_user_index  
        
    else:
        return G_act, user_index
...
def build_feat_label_data(G, ups, features, pixel=False, train_x_y=None):
    ...
    with open(G.unique_edges_file_name, 'r') as file_in:
        ...
        for r in reader:
            ...
            if type_edge > 0 and (train_x_y is None or ((x_u,y_u) in train_x_y and (x_v,y_v) in tran_x_y)):
                if pixel is True:
...
def compute_weight_multithread(edge_buffer, ups, model, features_file_name, n_threads):
    '''
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_threads) as executor:
        for t in range(n_threads):
            fut = executor.submit(compute_weight_wrapper, (edge_parts[t], ups, model, features_file_name))
            futures.append(fut)
...
def compute_edge_weights_multithread(G, ups, model, features, features_file_name, n_threads):
    '''
        ...
        for r in reader:
            ...
            if type_edge > 0:
                ...
                if len(edge_buffer) >= G.buffer_size:
                    ...
                    W = compute_weight_multithread(edge_buffer, ups, model, features_file_name, n_threads)
...
def embed_users_sign(G, ups, ndim=40, threshold=5, total_samples=100, n_negatives=5, n_iterations=10, compute_balance=False):
    '''
